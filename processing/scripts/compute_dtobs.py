...
index_new = ['ini error', 
             ...
             'ecl error',
             'DTmax 30min lineal', 
             'DTmax 30min spline interp', 
             'DTmax 30min spline smooth',
             'DTecl 30min lineal',
             'DTecl 30min spline interp',
             'DTecl 30min spline smooth', 
...
for col in columns:
    ...
    df.loc['DTmax 30min lineal', col] = round_float(dlineal)
    df.loc['DTmax 30min spline interp', col] = round_float(dinterp)
    df.loc['DTmax 30min spline smooth', col] = round_float(dsmooth)
    ...
    df.loc['Argmax 30min lineal', col] = to_str(calc_time(lineal))
    df.loc['Argmax 30min spline interp', col] = to_str(calc_time(interp))
    df.loc['Argmax 30min spline smooth', col] = to_str(calc_time(smooth))

    # calculate eclipse DT
    temp_ecl = temp.sel(time='2021-12-04 ' + df.loc['Maximum eclipse', col][:l], method='nearest')
    time_ecl = temp_ecl.time
    df.loc['DTecl 30min lineal', col] = round_float((lineal-origin).sel(time=time_ecl))
